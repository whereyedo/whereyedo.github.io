...
객체의 생성 및 초기화에 비용이 많이 들고, 이를 실제로 사용하는 시점이 불분명한 경우 해당 객체가 실제로 필요할때 생성되도록 지연시키는 방식
```java
// 이미지 표시를 위한 인터페이스
...
객체에 대한 접근을 제한하고 보안 검사를 수행해야 할 때 사용된다.
```java
// 파일을 읽기 위한 인터페이스
...
객체의 생성 및 초기화에 비용이 많이 들고, 동일한 요청이 반복되는 경우에는 매번 작업을 다시 수행하지 않고 캐싱된 결과를 반환하는 방식
```java
